				...
				if tc.Name == test {
					if excludeDiscreteRegex.MatchString(tc.Name) {
						if ret.Testcases[idx].Failure != nil {
							// Empty .Failure and add references to the test being skipped by the filter
							tcClass := ret.Testcases[idx].Classname
							ret.Testcases[idx] = junit.Testcase{}
							ret.Testcases[idx].Name = tc.Name
							ret.Testcases[idx].Classname = tcClass
							ret.Testcases[idx].Skipped = &junit.Result{Data: "Test output ignored via `exclude_discrete_tests`"}
							ret.Failures--
						}
						tc = junit.Testcase{}
						ret.Skipped++
